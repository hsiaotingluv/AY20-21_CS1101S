...
function choose(n, r) {
	if ((r < 0) || (n < 0) || (r > n)) { // Change this condition
		return 0;
	} else if (r === 0) { // Change this condition
		return 1;
	} else {
	    // DO NOT EDIT
		return DONT_CARE;
...
function choose(n, r) {
	if ((r < 0) || (n < 0) || (r > n)) { // Change this condition
		...
		return CHOOSE_FIRST_ITEM + NOT_CHOOSE_FIRST_ITEM; // YOUR SOLUTION HERE
	}
}
...
function sum_odd_lte(n) {
    // YOUR SOLUTION HERE
    return sum(x => (x % 2 === 1) ? x : 0, 1, x => x + 1, n);
}
